...
import { uploadFiles, storageFile } from "./helpers/file-upload.js";
...
export async function createServer(
  ...
  app.post("/api/storage", async (req, res) => {
    ...
    try {
      const data = await storageFile(session, req.body);
      res.status(status).send({ data, success: status === 200, error });
    } catch (e) {
      console.log(`Failed to create storage: ${e.message}`);
      status = 500;
      error = e.message;
    }
  });
  ...
  app.post("/api/files", async (req, res) => {
    ...
    try {
      const data = await uploadFiles(session, req.body);
      res.status(status).send({ data, success: status === 200, error });
    } catch (e) {
      console.log(`Failed to upload files: ${e.message}`);
      status = 500;
      error = e.message;
    }
  });
